...
export const loadHomePage = (): void => {
	...
	const usernameInput = document.getElementById("loginUsernameInput") as HTMLInputElement;
	...
	const orContinueSpan = document.querySelector('.relative.flex.justify-center.text-sm span');
	...
	const twoFAInput = document.getElementById("twoFACodeInput") as HTMLInputElement;
	...
	const setUsernameInput = document.getElementById("newUsernameInput") as HTMLInputElement;
	...
	submitBtn.onclick = async () => {
		...
		try {
			...
			if (res.ok) {
				...
				const data = await res.json();
				errorMsg.textContent = data.error || t("invalid_code");
