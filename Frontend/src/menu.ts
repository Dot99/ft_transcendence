...
export const loadMenuPage = async (): Promise<void> => {
    ...
    getElement<HTMLInputElement>('colorBoardBorder').addEventListener('input', e => {
    ...
    const closeJoinTournamentModal = document.getElementById('closeJoinTournamentModal');
    const tournamentList = document.getElementById('tournamentList');

    if (btnJoinTournament && joinTournamentModal && closeJoinTournamentModal && tournamentList && tournamentModal) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    tournaments = data.tournaments.filter(
                        (t: { players: number, maxPlayers: number }) => t.players < t.maxPlayers
                    );
                }
            } catch (e) {
    ...
    if (btnCreateTournament && createTournamentModal && closeCreateTournamentModal && tournamentModal) {
        ...
        closeCreateTournamentModal.addEventListener('click', () => {
            ...
            if (form) {
                ...
                const playerCountBtns = form.querySelectorAll<HTMLButtonElement>('.player-count-btn');
                playerCountBtns.forEach(b => {
                    b.classList.remove('bg-[#4CF190]', 'text-[#001B26]', 'ring-2', 'ring-[#4CF190]');
                    b.classList.add('bg-gray-600', 'text-white', 'border-gray-700');
                    b.classList.remove('border-[#001B26]');
                });
                const btn4 = form.querySelector<HTMLButtonElement>('.player-count-btn[data-value="4"]');
                if (btn4) {
                    btn4.classList.remove('bg-gray-600', 'text-white', 'border-gray-700');
                    btn4.classList.add('bg-[#4CF190]', 'text-[#001B26]', 'border-[#001B26]', 'ring-2', 'ring-[#4CF190]');
                }
                const playerCountInput = form.querySelector<HTMLInputElement>('#tournamentPlayers');
                if (playerCountInput) playerCountInput.value = '4';
            }
        });
    }

    const playerCountBtns = document.querySelectorAll<HTMLButtonElement>('.player-count-btn');
    const playerCountInput = document.getElementById('tournamentPlayers') as HTMLInputElement;
    ...
    if (playerCountBtns && playerCountInput) {
        playerCountBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                btn.classList.add('bg-[#4CF190]', 'text-[#001B26]', 'border-[#001B26]', 'ring-2', 'ring-[#4CF190]');
