...
function attachProfileEventListeners(user: User, twofaEnabled: boolean) {
	...
	getElement<HTMLFormElement>("editProfileForm").onsubmit = async (e) => {
		...
		let twofa_enabled = enable2faCheckbox
			? enable2faCheckbox.checked
			: false;
		const userId = getUserIdFromToken();
		...
		if (res.ok) {
			// If enabling 2FA (checkbox checked and wasn't enabled before)
			if (twofa_enabled && !twofaEnabled) {
			...
			// If disabling 2FA (checkbox unchecked and was enabled before)
			else if (!twofa_enabled && twofaEnabled) {
