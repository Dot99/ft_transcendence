...
export const loadProfilePage = (pushState: boolean = true): void => {
	...
	const deleteModalMsg = document.querySelector("#deleteModal .text-gray-200");
	if (deleteModalMsg)
	...
	const editUsernameLabel = document.querySelector("label[for='editUsername']");
	...
	if (recentMatchesTitle) recentMatchesTitle.textContent = t("recent_matches");
	const pastTournamentsTitle = document.getElementById("pastTournamentsTitle");
	if (pastTournamentsTitle)
		pastTournamentsTitle.textContent = t("tournament_past");
	const upcomingMatchesTitle = document.getElementById("upcomingMatchesTitle");
