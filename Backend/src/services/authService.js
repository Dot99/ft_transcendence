...
export async function handleGoogleCallback(request, fastify) {
	try {
		...
		try {
			...
			// If state is invalid, provide a more specific error
		...
		if (!user) {
			let rawUsername = profile.name || profile.email.split("@")[0];
			// Remove spaces
			let cleanedUsername = rawUsername.replace(/\s+/g, "");
			// Check if there's at least one digit
			if (!/\d/.test(cleanedUsername)) {
				const randomDigit = Math.floor(Math.random() * 10);
				cleanedUsername += randomDigit;
			}
			...
			user = await dbGet("SELECT * FROM users WHERE id = ?", [
				result.lastID,
			]);
		}
...
export async function twoFaVerifyService(userId, enabled) {
	try {
		const result = await dbRun("SELECT * FROM users WHERE id = ?", [
			userId,
		]);
