...
export function createUser(username, password, country, lang = "en") {
	return new Promise((resolve) => {
		if (!username || !password || !country) {
			return resolve({
				success: false,
				message: messages[lang].missingFields,
			});
		}
...
export async function login(username, password, fastify, lang = "en") {
	return new Promise(async (resolve) => {
		...
		if (!user.success) {
			return resolve({
				success: false,
				message: messages[lang].invalidLogin,
			});
		}
		...
		if (isUserActive(user.user.id)) {
			return resolve({
				success: false,
				message:
					messages[lang].alreadyLoggedIn ||
...
export function register(username, password, country, fastify, lang = "en") {
	return new Promise(async (resolve) => {
		...
		if (existingUser && existingUser.success) {
			return resolve({
				success: false,
				message: messages[lang].userExists,
			});
		}
...
export function getUserFriends(userId, lang = "en") {
	return new Promise((resolve, reject) => {
		if (!userId) {
			return resolve({
				success: false,
				message: messages[lang].noUserId,
			});
		}
		db.all(
			...
			(err, rows) => {
				...
				if (!rows || rows.length === 0) {
					return resolve({
						success: false,
						message: messages[lang].noFriends,
					});
				}
		...
		db.serialize(() => {
			db.get(
				...
				(err, existingEntry) => {
					if (err) {
						console.error(
							`[MM] DB error on select for user ${userId}:`,
							err
						);
						return reject(err);
					}
					...
					if (existingEntry) {
						if (existingEntry.status === "matched") {
							return resolve({
								success: true,
								matched: true,
								gameId: existingEntry.game_id,
								opponentId: null, // You can fetch opponent if needed
					...
					function afterCleanup() {
						db.get(
							...
							(err, waitingOpponent) => {
								if (err) {
									console.error(
										`[MM] DB error finding opponent for user ${userId}:`,
										err
									);
									return reject(err);
								}
								if (waitingOpponent) {
										...
										(err, stillWaiting) => {
											...
											db.run(
												...
												function (err) {
													if (err) {
														console.error(
															`[MM] DB error updating opponent for user ${userId}:`,
															err
														);
														return reject(err);
													}
													...
													db.run(
														...
														function (err) {
															if (err) {
																console.error(
																	`[MM] DB error inserting matched for user ${userId}:`,
																	err
																);
																return reject(
																	err
									...
									db.run(
										...
										function (err) {
											if (err) {
												console.error(
													`[MM] DB error inserting waiting for user ${userId}:`,
													err
												);
												return reject(err);
											}
	...
	return new Promise((resolve, reject) => {
		db.get(
			...
			(err, userStatus) => {
				if (err) {
					console.error(
						`[MM] DB error getting user status for leave:`,
						err
					);
					return reject(err);
				}
				if (userStatus && userStatus.game_id) {
					db.run(
						...
						function (err) {
							if (err) {
								console.error(
									`[MM] DB error deleting by game_id for user ${userId}:`,
									err
								);
								return reject(err);
							}
					...
					db.run(
						...
						function (err) {
							if (err) {
								console.error(
									`[MM] DB error deleting by user_id for user ${userId}:`,
									err
								);
								return reject(err);
							}
	...
	return new Promise((resolve, reject) => {
		db.get(
			...
			(err, row) => {
				if (err) {
					console.error(
						`[MM] DB error on status for user ${userId}:`,
						err
					);
					return reject(err);
				}
				...
				if (row.status === "matched" && row.game_id) {
					db.get(
						...
						(err, opponent) => {
							if (err) {
								console.error(
									`[MM] DB error getting opponent for user ${userId}:`,
									err
								);
							} else if (opponent) {
